# Avellaneda Perpetual Market Making Strategy Cython Header
#
# This file is nearly identical to the spot Avellaneda .pxd file with minimal changes:
#
# KEY DIFFERENCES FROM SPOT VERSION:
# 1. Class name changed to AvellanedaPerpetualMarketMakingStrategy
# 2. All internal variables and methods remain identical (same Avellaneda-Stoikov algorithm)
# 3. The strategy logic, volatility calculation, and optimal spread computation are unchanged
...
cdef class AvellanedaPerpetualMarketMakingStrategy(StrategyBase):
    # All cdef variables are identical to spot version - Avellaneda algorithm is the same
    cdef:
        object _config_map
        object _market_info
    ...
    # All method signatures are identical to spot version - same algorithm implementation
