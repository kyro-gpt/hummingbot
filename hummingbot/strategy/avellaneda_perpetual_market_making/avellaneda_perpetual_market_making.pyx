...
# It is adapted from the spot Avellaneda strategy with minimal changes for derivative trading:
#
# KEY DIFFERENCES FROM SPOT VERSION:
# 1. Uses DerivativeBase instead of ExchangeBase for perpetual connector integration
# 2. Adds PositionAction.OPEN to all order executions for perpetual position management
# 3. Uses derivative-specific configuration and validation
# 4. Supports leverage trading (configured in config map)
#
# IDENTICAL TO SPOT VERSION:
# - All Avellaneda-Stoikov mathematical formulas and parameters (γ, η, volatility calculation)
# - Optimal spread and reservation price calculations
# - Inventory management and order refresh logic
# - Risk management and order optimization
# - Hanging orders and multi-level order support
#
# This strategy is designed for ONE-WAY position mode only to minimize complexity
# while maintaining the full power of the Avellaneda-Stoikov pricing model.
...
# PERPETUAL DIFFERENCE: DerivativeBase inherits from ExchangeBase, so we use ExchangeBase for Cython compatibility
from hummingbot.connector.exchange_base import ExchangeBase
from hummingbot.connector.exchange_base cimport ExchangeBase
from hummingbot.core.clock cimport Clock

from hummingbot.client.config.config_helpers import ClientConfigAdapter
# PERPETUAL DIFFERENCE: Add PositionAction for perpetual order execution
...
# PERPETUAL DIFFERENCE: Import perpetual-specific config map
...
# PERPETUAL DIFFERENCE: Use local data_types for consistency with perpetual strategies
from hummingbot.strategy.avellaneda_perpetual_market_making.data_types import (
    PriceSize,
    Proposal,
)
...
# PERPETUAL DIFFERENCE: Class name for perpetual version, but algorithm logic remains identical
cdef class AvellanedaPerpetualMarketMakingStrategy(StrategyBase):
    ...
    def init_params(self,
                    # PERPETUAL DIFFERENCE: Use perpetual config type
                    config_map: Union[AvellanedaPerpetualMarketMakingConfigMap, ClientConfigAdapter],
    ...
    def order_levels(self) -> int:
        if self._config_map.order_levels_mode.model_config["title"] == MultiOrderLevelModel.model_config["title"]:
    ...
    def get_config_map_execution_mode(self):
        try:
            execution_mode = self._config_map.execution_timeframe_mode.model_config["title"]
    ...
    def get_config_map_hanging_orders(self):
        if self._config_map.hanging_orders_mode.model_config["title"] == TrackHangingOrdersModel.model_config['title']:
    ...
    cdef c_apply_budget_constraint(self, object proposal):
        # PERPETUAL DIFFERENCE: Use budget_checker for margin/leverage constraints instead of manual balance checking
        # This allows proper shorting and leverage handling in perpetual trading
    ...
    def create_order_candidates_for_budget_check(self, proposal: Proposal):
        # PERPETUAL DIFFERENCE: Create PerpetualOrderCandidate with leverage for proper margin calculations
        ...
        order_candidates.extend([
            PerpetualOrderCandidate(
                ...
                buy.size,
                buy.price,
                leverage=Decimal(str(self._config_map.leverage)),
            )
    ...
    def apply_adjusted_order_candidates_to_proposal(self, adjusted_candidates, proposal: Proposal):
        # PERPETUAL DIFFERENCE: Handle margin-adjusted order candidates, allows zero orders for insufficient margin
        ...
        for order in chain(proposal.buys, proposal.sells):
            ...
            if adjusted_candidate.amount == s_decimal_zero:
                self.logger().info(
                    f"Insufficient margin: {adjusted_candidate.order_side.name} order (price: {order.price}, "
                    f"size: {order.size}) is omitted."
                )
                self.logger().warning(
                    "You are at possible risk of liquidation if there is an open loss."
                )
    ...
    cdef c_apply_add_transaction_costs(self, object proposal):
        cdef:
            ExchangeBase market = self._market_info.market
        for buy in proposal.buys:
            # Use c_get_fee() method same as spot strategy - no PositionAction needed for fee calculation
    ...
    cdef c_execute_orders_proposal(self, object proposal):
        ...
        if len(proposal.buys) > 0:
            ...
            for idx, buy in enumerate(proposal.buys):
                # PERPETUAL DIFFERENCE: Add position_action=PositionAction.OPEN for perpetual orders
